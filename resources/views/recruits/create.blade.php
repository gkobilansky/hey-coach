...
@section('heading')
    <h1>{{ __('Create recruit') }}</h1>
@stop

@section('content')

    {!! Form::open([
            'route' => 'recruits.store'
            ]) !!}
    ...
    <div class="form-group">
        ...
        @else
            {!! Form::label('athlete_id', __('Assign athlete'), ['class' => 'control-label']) !!}
            {!! Form::select('athlete_id', $athletes, null, ['class' => 'form-control']) !!}
        @endif
    </div>

    {!! Form::submit(__('Recruit'), ['class' => 'btn btn-primary']) !!}
